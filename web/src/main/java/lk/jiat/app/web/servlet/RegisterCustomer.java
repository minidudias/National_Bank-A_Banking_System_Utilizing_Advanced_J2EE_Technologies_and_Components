...
public class RegisterCustomer extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            customer.setJoinedDate(new Date(System.currentTimeMillis()));
            customer.setVerifiedStatus(VerifiedStatus.UNVERIFIED);
            customer.setVerificationCode(verificationCode);

            Thread emailSenderThread = new Thread(){
                @Override
                public void run(){
                    Mail.sendMail(email, "National Bank Customer Verification",
            ...
            jsonResponse = Json.createObjectBuilder()
                    .add("status", "success")
                    .add("message", "Customer registered successfully. Temporary code was sent to their email. : " + email)
                    .build();
