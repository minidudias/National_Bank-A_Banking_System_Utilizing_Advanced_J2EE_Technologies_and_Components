...
<%
    ...
    try {
        ...
        customers = userService.getAccountHoldableUsers();
...
<head>
    ...
    <title>National Bank - Banker's Dashboard</title>
...
<div class="navbar navbar-expand-lg navbar-dark bg-primary">
    <div class="container">
        <a class="navbar-brand d-flex align-items-center" href="#">
            ...
            National Bank - Banker's Dashboard
        </a>
...
<div class="container mt-4">
    <div class="card shadow-sm">
        <div class="card-header bg-light">
            <h4 class="mb-0 text-primary">Customer Management</h4>
        </div>
        <div class="card-body">
            <div class="register-form">
                <h5 class="mb-3">Register a New Customer</h5>
                <form id="registerCustomerForm" action="${pageContext.request.contextPath}/register-customer" method="post">
                    <div class="row g-3">
                        <div class="col-md-3">
            ...
            <h5 class="mt-4 mb-3">Registered Account Holdable Users</h5>
            <% if (customers.isEmpty()) { %>
            <div class="alert alert-info" role="alert">
                No account holdable user registered yet.
            </div>
            <% } else { %>
            <div class="customers-list">
                ...
                <div class="customer-card">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h6 class="mb-1"><%= customer.getName() %></h6>
                            <div class="text-muted small"><%= customer.getEmail() %></div>
                            <div class="text-muted small"><%= customer.getContact() %></div>
                            <div class="text-muted small">NIC: <%= customer.getNic() %></div>
                        </div>
                        <div>
                <span class="verified-badge verified-<%= customer.getVerifiedStatus() %>">
                    <%= customer.getVerifiedStatus() %>
                </span>
                            <form action="open_accounts.jsp" method="post" class="manage-accounts-form">
                                <input type="hidden" name="customerId" value="<%= customer.getId() %>">
                                <button type="submit" class="btn btn-sm btn-outline-primary" style="margin-top: 10px;">
                                    <i class="fas fa-wallet me-1"></i> Manage Accounts
                                </button>
                            </form>
                        </div>
                    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        console.log('Banker Dashboard Loaded');
    ...
    document.querySelectorAll('.manage-accounts-form').forEach(form => {
        form.addEventListener('submit', function(e) {
            ...
            customerIdInput.value = this.querySelector('input[name="customerId"]').value;
            hiddenForm.appendChild(customerIdInput);
